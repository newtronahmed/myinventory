...
<script  >
    ...
    if($('#categoryManageModal')){
        $('#categoryManageModal').on('show.bs.modal',function(event){
            let triggerBtn = $(event.relatedTarget)
            let data = triggerBtn.data('data')
            let modal = $(this)
            modal.find('#categoryName').val(data.categories)
            // update brand
            modal.find('#categoryForm').attr('action','/category/edit/'+data.id)
            // update category
        })
